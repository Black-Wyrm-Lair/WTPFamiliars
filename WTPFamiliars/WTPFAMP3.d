...
IF ~InPartySlot(LastTalkedToBy,0)~ THEN BEGIN famtalk
  ...
  // Say goodbye
  IF ~CombatCounterLT(1)!TriggerOverride(%famsummoner%,Detect([ENEMY]))!Global("WTPFAMLG","GLOBAL",2)OR(2)ReputationGT(%famsummoner%,15)ReputationLT(%famsummoner%,6)~ THEN REPLY @27 GOTO famcomplain
...
IF ~~ THEN BEGIN fampet
  ...
  // Say goodbye
  IF ~!Global("WTPFAMLG","GLOBAL",2)OR(2)ReputationGT(%famsummoner%,16)ReputationLT(%famsummoner%,5)~ THEN REPLY @48 GOTO famcomplain
