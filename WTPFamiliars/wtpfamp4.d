...
IF ~~ THEN BEGIN fam1intopack
  SAY @30
  IF ~~ THEN DO ~SetGlobal("#GEMPCHA","GLOBAL",1)SetGlobal("#GSKIRES","GLOBAL",1)GivePartyAllEquipment()ReallyForceSpellRES("wtpclear",Myself)GiveItemCreate("wtpfmspd",%famsummoner%,0,0,0)SetGlobalTimer("#GWANOUT","GLOBAL",ONE_HOUR)SetGlobalTimer("","GLOBAL",FOUR_HOURS)DestroySelf()~ EXIT
END
...
IF ~~ THEN BEGIN fam1hurt_intopack
  SAY @16
  IF ~~ THEN DO ~SetGlobal("#GEMPCHA","GLOBAL",1)SetGlobal("#GSKIRES","GLOBAL",1)GivePartyAllEquipment()GiveItemCreate("wtpfmspd",%famsummoner%,0,0,0)SetGlobalTimer("#GWANOUT","GLOBAL",ONE_HOUR)SetGlobalTimer("","GLOBAL",FOUR_HOURS)DestroySelf()~ EXIT
END
