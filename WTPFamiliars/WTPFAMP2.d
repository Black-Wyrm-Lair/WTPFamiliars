...
IF ~InPartySlot(LastTalkedToBy,0)~ THEN BEGIN famtalk
  ...
  // Say goodbye
  IF ~CombatCounterLT(1)!TriggerOverride(%famsummoner%,Detect([ENEMY]))ReputationGT(%famsummoner%,15)~ THEN REPLY @27 GOTO fam1complain
...
IF ~~ THEN BEGIN fampet
  ...
  // Say goodbye
  IF ~ReputationGT(%famsummoner%,17)~ THEN REPLY @48 GOTO fam1complain
...
IF ~~ THEN BEGIN fam1hurt_intopack
  SAY @16
  IF ~~ THEN DO ~SetGlobal("#GEMPCHA","GLOBAL",1)GivePartyAllEquipment()ReallyForceSpellRES("wtpclear",Myself)Wait(1)GiveItemCreate("wtpfmimp",%famsummoner%,0,0,0)SetGlobalTimer("#GWANOUT","GLOBAL",300)SetGlobalTimer("#GSTAOUT","GLOBAL",1200)DestroySelf()~ EXIT
END
